...
4. 通过文案节点生成并编辑商品标题、卖点、海报主标题和 CTA。
5. 通过生图节点生成图片，并把结果填充到下游参考图槽位。
6. 下载图片/海报，或在右侧 Images 面板回看商品素材历史。
...
7. 移动端使用单栏布局：顶部操作、状态信息、结果预览、生成设置、参考图和参数选择按顺序展开。

### 3.3 商品 DAG 工作流

1. 在商品详情中打开工作流工作台。
2. 创建或调整节点：商品上下文、参考图、文案生成、图片生成。
3. 使用内置节点组模板追加常用流程，或多选节点保存为用户节点组模板。
4. 连接节点形成 DAG。
5. 启动后台工作流运行，必要时取消运行或重试失败运行。
6. 在数据库中持久化运行、节点状态和失败原因。
7. 运行中前端轮询轻量 workflow status；结束后刷新完整工作流、商品详情和历史产物。
...
- `CanvasTemplate` / `UserCanvasTemplate`：内置完整画布模板、内置节点组模板和用户保存的节点组模板。
...
5. 使用完整画布模板创建商品，并在工作台中插入内置或用户保存的节点组模板。
