...
工作台当前是 **ProductFlow workbench**：中间是可缩放、可拖动的节点画布，右侧是一个精简侧栏，侧栏用小 rail 在 **Details / Runs / Images / 模板** 之间切换。普通操作不需要理解内部 DAG，只要记住：商品、参考图、文案、生图都是卡片；连接线表示“下游生成时参考上游”。
...
- **多选卡片**：在画布空白区域拖出选择框，或使用多选操作选中多个节点。多选后可以一起拖动、删除，也可以保存为节点组模板。
- **调整侧栏**：右侧侧栏负责 Details、Runs、Images 和模板；它会尽量保持精简，不再用底部大面板占用画布空间。

### 节点组模板

右侧 **模板** 面板用于在已有商品工作台中插入一组预设节点。它和新建商品时的完整画布模板不同：

- **完整画布模板**：只在新建商品时选择，用来决定初始工作流结构。
- **节点组模板**：在商品工作台中追加一段流程，例如主图细化、场景图或活动图。
- **用户模板**：多选两个或更多非商品资料节点后，可以把当前结构保存成自己的节点组模板。
...
- 运行中的工作流可以在包含该节点的 Details 或 Runs 区域取消；失败且可重试的运行会提供重试入口。
...
确认已经多选至少两个节点，并且不要把 **商品** 节点一起保存。用户节点组模板只保存可复用流程片段，不能包含商品资料节点，也不会保存已经生成的图片或文案结果。
