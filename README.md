...
- 商品列表、分页浏览、创建商品、商品详情工作台、受全局开关保护的商品删除。
- 节点画布组织商品资料、参考图、文案节点和生图节点。
- 画布支持滚轮缩放、空白处拖动平移、节点拖拽定位、节点连线、边删除、Ctrl/Cmd/Shift 多选、Shift 框选。
...
- 文案节点支持生成、编辑、确认和历史查看，字段包括标题、卖点、海报主标题和 CTA。
- 生图节点只负责触发和配置生成；生成结果写入连接的下游参考图节点，并在参考图/图片侧栏预览和下载。

### 文/图生图

- 独立图片会话支持参考图上传、历史基图选择、连续生成、多候选比较和移动端单栏布局。
...
![工作台图片面板示例](images/preview3.png)

![生成结果示例](images/preview4.png)
...
- `IMAGE_PROVIDER_KIND=openai_responses`：OpenAI Responses `image_generation` 工具，支持参考图输入。ProductFlow 当前的连续生图分支上下文由用户显式选择的基图和参考图决定，不会自动把整段历史图片都传给 provider。
- 相关变量：`IMAGE_API_KEY`、`IMAGE_BASE_URL`、`IMAGE_GENERATE_MODEL`、`IMAGE_GENERATION_MAX_DIMENSION`、`IMAGE_MAIN_IMAGE_SIZE`、`IMAGE_PROMO_POSTER_SIZE`。
